 ...
 *  class        property
 *
 *  ChessBoard - tableMap: array for each cell in the table
 ...
 *  Units
 * */
var IMG = "/resources/app/game/chess/img/";
...
class ChessBoard {
    ...
    setupBoard() {
        var chessContainer = document.querySelector("#chess-container");
        chessContainer.style.marginTop = "20px";

        var boardSection = document.createElement("section");
        boardSection.setAttribute("id", "chessBoard");
        chessContainer.appendChild(boardSection);

        var chessBoard = document.createElement("table");
        chessBoard.setAttribute("cellpadding", "0");

        chessBoard.style.margin = "auto";
        chessBoard.style.height = 480 + "px";
        chessBoard.style.width = 480 + "px";
    ...
    changeTurn() {
        ...
        if (this.whoseTurn == "W") {
            for (var i = 0; i < this.playerW.units.length; i++)
                this.playerW.units[i].unit.setAttribute("class", "Disable");

            for (var i = 0; i < this.playerB.units.length; i++)
        ...
        // right after playerB action
            ...
            for (var i = 0; i < this.playerW.units.length; i++)
                this.playerB.units[i].unit.setAttribute("class", "Disable");
...
class Unit {
    ...
    putUnit(player, id, row, column, tableMap) {
        ...
        if (id.charAt(0) == "W")
            unit.setAttribute("class", "Enable");
        else
            unit.setAttribute("class", "Disable");
    ...
    isInCheck() {
        // return boolean
        this.showWays();
        var enemies = document.querySelectorAll(".AbleToAttack");
        Unit.removeWays();
    ...
    // remove ways
    static removeWays() {
        var ways = document.querySelectorAll(".AbleToMove");
    ...
    checkWay(row, column) {
        ...
        if (row < 0 || row > 7 || column < 0 || column > 7) {
            ...
            this.tableMap[row][column].setAttribute("class", "AbleToMove");
            return 1;
        }
    }
    ...
    checkEnemy(row, column) {
        if (this.id.charAt(0) != this.tableMap[row][column].children[0].id.charAt(0)) {
            this.tableMap[row][column].setAttribute("class", "AbleToAttack");
            return true;
        }
        return false
    }
        ...
        if (this.id.charAt(0) == "W") {
            ...
            if (this.moveCount == 0) {
                if (!this.tableMap[row - 1][column].hasChildNodes())
                    this.tableMap[row - 1][column].setAttribute("class", "AbleToMove");
                if (!this.tableMap[row - 2][column].hasChildNodes())
                    this.tableMap[row - 2][column].setAttribute("class", "AbleToMove");
            }
